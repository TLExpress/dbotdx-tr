...
class resource_provider_t:
    ...
    async def fetch_init(self):
        ...
        if (
            ...
            or self.station_table_tomorrow is None
            or self.train_table is None
            or self.train_table_tomorrow is None
    ...
    async def fetch_daily(self):
        self.station_table, self.station_table_tomorrow, self.train_table, self.train_table_tomorrow = (
            await asyncio.gather(
                station_table_t(self._requester).fetch(),
                station_table_t(
                    self._requester,
